...
## Creature concept learnings

- **2026-08-16 — Bellhide gate A: MATERIAL PASS, CREATURE FAIL.** Leonardo V2 Phoenix 1.0 FAST, 1024x1024, seed `731942`, one private image, actual cost `$0.015`. The cast bronze, weathered cracks, slate-blue hide and shared cyan rune/eye palette rendered clearly. The structural brief did not: strict side profile became a three-quarter view, the compact recurring mob became a large wolf-like beast, three broken ward-bell fragments became several complete bells with handles, and the clapper-tail became an ordinary curled tail. **Lesson:** repeating `bell` as a concrete material noun overwhelms the more abstract `fragment/carpace` relationship; Phoenix instantiates literal bells. Preserve only the metal/rune palette as optional design DNA.

- **2026-08-16 — Bellhide gate B: HARD FAIL; stop text-only retries.** Leonardo V2 Phoenix 1.0 FAST, 1024x1024, seed `731943`, one private image, actual cost `$0.015`. A targeted prompt removed complete bells and canine anatomy, but Phoenix swung to a giant rear-facing humanoid brute in decorative armor. It lost the side profile, small-mob proportions, four-legged anatomy, visible eye, rune identity and resonant mantle. **Lesson:** `powerful limbs + hairless + mantle` activates Phoenix's muscular humanoid prior even when quadruped, side elevation and negatives are explicit. Do not buy a third text-only retry. The next Bellhide input should be a user-refined Leonardo composition or a concrete sketch/image reference, then PixelLab should translate that approved identity.

- **2026-08-16 — Bellhide gate C: CONCEPT COMBINATION PASS, ANGLE FAIL.** Leonardo V2 Phoenix 1.0 FAST, 1024x1024, seed `731944`, one private image, actual cost `$0.0179`. Gate A was supplied as a LOW style/material reference and gate B as a LOW image-to-image anatomy reference. The combination worked: a heavy non-dog biped, bronze bell pieces, cyan runes and removable-object potential all survived. The camera did not: even with orthographic side-elevation language, one visible eye, overlapping far limbs, and explicit rear/three-quarter negatives, Phoenix preserved gate B's rear three-quarter pose and hid the face. Leonardo's current official guidance matrix explains the wall: Phoenix supports Style, Character and Content Reference, but **not Pose, Edge or Depth guidance**; Pose-to-Image is available only on older SD/SDXL model lanes. Therefore another Phoenix prompt cannot guarantee a game-ready side profile. Keep gate C for user refinement, but require a side-facing sketch/reference or a Leonardo web-tool pose correction before PixelLab.

## Environment composition learnings

- **2026-08-16 — Castle Front side-view gate A: STRUCTURAL FAIL, COLOR/MOOD PASS.** Leonardo V2 Phoenix 1.0 FAST, 1280x720, seed `417203`, one private image, actual cost `$0.015`. The prompt said "fixed straight-on side camera" and "one perfectly level horizontal walking surface," but the later phrase "The road leads east" won: Phoenix rendered a modern-looking road with yellow centre markings receding into a strong central vanishing point. It also let castle masonry spill beyond the left-quarter boundary. The warm lavender castle, golden sky, distant blue mountains, and welcoming adventure mood all worked and should be preserved. **New side-scroller rule:** describe the playable ground as a visible vertical soil-and-stone cut face beneath one straight horizontal top edge; say that every scenery layer runs laterally in parallel bands; explicitly prohibit receding roads, converging ground lines, central vanishing points, highways, asphalt, and road markings. Never use "the road leads into the landscape" for a side-view plate. Composition nouns and visible geometry beat camera-angle adjectives.

- **2026-08-16 — Castle Front scope correction: BACKGROUND PLATE ONLY.** The generated castle was not a desired subject: the project already has castle and object assets from PixelLab, and Phaser will place them as independent foreground layers. Preserve only gate A's successful warm dusk palette and distant scenic mood—not its castle content. The Leonardo environment prompt must request sky, distant natural layers and one left-to-right ground band, leaving the foreground empty and the left third quiet for the separate facade. During angle calibration, every provider request and every harness run is capped at exactly one image; no two-, three- or four-image batches until the background-only prompt and side elevation are visually approved.

- **2026-08-16 — Superseding Castle Front pipeline: GENERATE PARALLAX LAYERS, NOT A STRUCTURED SCENE.** The whole-plate prompt is retired before generation. Do not reserve castle space, combat space, a road or a ground line inside Leonardo background work; those are Phaser composition concerns. Generate and approve each layer independently: first a full-canvas opaque sky, then far-mountain strips, then nearer mountain/hill strips, then modular distant architecture such as castles and houses. The visual harness must show every isolated layer, its pixel preview, a horizontal continuity/seam test, and a composite preview over already-approved layers. Continue the one-image calibration rule for each layer family.

- **2026-08-16 — Sky layer 01: CLEAN LAYER ISOLATION, HUMAN REVIEW.** Leonardo V2 Phoenix 1.0 FAST, 1280x720, seed `417203`, one private image, actual cost `$0.015`. The full-canvas sky-only instruction worked: no land, mountains, architecture, actors or objects appeared, and the blue/peach/gold/lavender cloud language survived the deterministic 320x180 pixel conversion beautifully. Two judgment points remain: Phoenix moved the requested right-third sun to the left edge, and the lower cloud bank is dense enough that it may compete with later silhouette layers. Record this as `HUMAN_REVIEW`, not approved, until Raheem decides whether those traits belong in the retained sky anchor.
