...
- **Use `mode: 'v3'` with per-direction `custom_start_frame` pinning. This is the default for everything.** Each direction's clip is pinned to that direction's own rotation image, which is what holds both the facing and the identity. Note v3 returns `frame_count + 1` frames — frame 0 is the pinned reference pose, so the cycle proper starts at index 1. `frame_count` must be **even and ≥ 4**; odd values are rejected (for free).
- **Template mode drifts.** It reconstructs each direction independently and produced a rogue frame whose costume sat 43.7 palette units from its own cycle, and it **lost the front facing entirely** — `north` and `south` both came back as back views. Keep it for a stationary character's `breathing-idle` (1 generation, `south` only, where drift across a subtle loop does not matter). Never for a walk.
- **`pro` mode is BANNED. It cost ~186 generations for one character and the result was unusable** — corrupt frames with grey background artifacts, both side rows facing the same way (mirror check failed: same 0.542 vs flipped 0.503), and it ignored the requested frame count. Wreckage preserved at `scripts/sprite-lab/out/hero-chibi-v1-pro-failed/`. The same character redone with **v3 + pinning cost ~25 generations and passed the quality gate on the first attempt** — an order of magnitude cheaper *and* correct.
  - *This entry previously recommended `pro` as the default.* That advice predated the run above and was wrong; anyone who followed it burned a tenth of the monthly allowance on one broken character. Corrected 2026-07-31.
  - **Do not confuse `mode: 'pro'` (an ANIMATION mode — banned) with the `/create-character-pro` ENDPOINT (character creation from a design reference — good, and how both bosses were made).** Same word, different jobs. `create-boss` uses the endpoint and is unaffected by this ban.
