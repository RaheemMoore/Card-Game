...
Both are traced by a human in Figma. Codex reads them and generates everything
else.

## Why a human traces

Codex has failed at this automatically three separate ways, each costing a round
...
Codex also traced coordinates by cropping and measuring twice, and was wrong
...
**Division of labour: Raheem decides where objects begin and end. Codex does
...
- **Names are the contract.** An occluder and its collider share a name. Codex
...
4. **`groundY` falls out of the trace** — for an accurate silhouette, the bottom
   of the shape is where the object meets the floor. This is the number Codex
