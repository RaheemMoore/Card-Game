...
> **Studio control plane:** Read [AI_STUDIO_ARCHITECTURE.md](AI_STUDIO_ARCHITECTURE.md) for the simple operating model and `.claude/studio/STUDIO_CAPABILITY_REGISTRY.json` for routing/status. Choose FAST, STANDARD, or FULL mode. Agents advise; skills execute; the primary Codex or Claude Code session integrates. Runtime/visual work returns PASS, FAIL, or HUMAN REVIEW. Shared policy lives under `.claude/`; Codex adapters live under `.agents/` and `.codex/`.
...
Requires a `.env` file in `card-engine/` with the client keys (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`) and, for `vercel dev` or preview deploys, the server keys (`ANTHROPIC_API_KEY`, `LEONARDO_API_KEY`, `SUPABASE_SERVICE_ROLE_KEY`). Card text generation runs on Codex Haiku 4.5 via the server-side `/api/anthropic-messages` endpoint. Portrait art runs on Leonardo Phoenix via `/api/leonardo` (server-side key too).
...
- **Anthropic Codex Haiku 4.5** for card name/title/lore/portrait-prompt generation via `/api/anthropic-messages`
...
│   │   │   ├── claudeApi.ts        # Bible-driven prompt (Bible §Codex Generation Pipeline 14 steps)
...
interface Card {
  ...
  hiddenFate?: HiddenFate;               // Codex-inferred; locked identity fields preserved across ranks
...
5. **Forge** — Calls Codex API following Bible §Codex Generation Pipeline (14 steps): archetype chapter → answers → element+bond → classify tensions → Hidden Fate → visual summary → Leonardo prompt ≤1300 chars. Portrait via Leonardo. Reveals with fade-in.
...
- `.claude/agents/` is the canonical set of read-only specialist directors; `.codex/agents/` contains the equivalent Codex adapters with a read-only sandbox. Invoke only when the registry trigger matches an open decision; FAST work normally uses none.
- `.claude/skills/` is the canonical workflow set; `.agents/skills/` contains Codex-discoverable adapters that point back to the same contracts and scripts. `balance-playtest` remains hidden/inactive; `ship-minigame` and the completed `extract-fullscreen-shell` migration are retired and hidden.
...
- `.claude/settings.json` + `.claude/scripts/` — Claude Code permissions and shared policy enforcement. `.codex/config.toml` + `.codex/hooks.json` adapt those protections for Codex.
