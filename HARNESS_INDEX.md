...
| **Battle ability + decision sequence** | `/dev/seed-battle` → `/battle`; scenario `battle-ability-decision-sequence` | Does the combined system work in the authentic fight rather than a reconstructed mockup? DEV-only `window.__CARD_ENGINE_BATTLE_STUDIO__` plus DOM observation attributes report reducer phase/battle id, current and queued beats, input lock, motion, live performance id/form/stage, caster and target ids, named anchor coordinates, and decision-surface visibility. The browser still drives the real picker, wallet, ability controls, reducer, journal, and boss. |
