...
**THE ANGLE CHANGED ON 2026-08-16: the game is a 2D side-scroller.** Flat side elevation,
camera at ground level looking straight on. **[SIDE_VIEW_ANGLE_SPEC.md](SIDE_VIEW_ANGLE_SPEC.md)
is canon** — read it before generating or placing anything, and apply its one-second test: *if
you can see the TOP of a thing, it is the wrong angle.*

This replaces the old `low top-down` lock, which existed because that angle was paid for. Raheem
retired it: four directions multiplied every character performance by four, which is
*"a bit extreme stressful"* for a first game, and the budget belongs in dramatic card magic
instead. **Only three existing assets survive the change** — the Ember Jelly (symmetric, zero
directions), the 545 elemental effect strips (no baked facing), and the hero's left/right walk
and card-blast rows. Everything else in the repo is the wrong angle and does not go in.

**A config's `view` field is a claim, not evidence — open the PNG.** The Halo Stone
`wall-straight-v3` and `tower-corner-v3` are labelled `view: "side"` and are plainly
three-quarter; the tower shows its whole top face. They were placed in the side-view scene on
the strength of that label and had to be pulled straight back out. Note also that the enum
**differs per endpoint**: character routes take `low top-down | high top-down | side`, object and
tile routes take `top-down | sidescroller`, and `/create-image-pixen` takes `side` but needs the
angle restated in prose as well — it is the route that produced the mislabelled kit art.
...
- **Castle front (2D side-scroller hub)** — `/castle` is `src/pages/castle/front-v4/`, a flat side-elevation castle exterior in Phaser 3. The Card-wright walks left and right and **cannot jump**; he cycles four carried cards with the wheel or `1–4`, holds `F` to charge and releases to fire horizontally; the Ember Jelly telegraphs a **committed leap that lands on the ground he was standing on**, so standing still is punished and both running past it and running away are real answers. A strong hit knocks him down and scatters the whole hand onto separated, reachable ground; proximity picks cards back up. `CastleFront.tsx` is the production shell (pause menu, forge indicators, hand readout); `/dev/castle-front-v4` is the same world wearing instruments and ships in no player build.

  **The top-down courtyards were DELETED on 2026-08-16** — CourtyardV2/V3, the classic plate castle, the v2-preview, the sample, their Editor scene files and the dev scene tooling that served them. Git history keeps them; do not resurrect them. Raheem: *"I definitely don't wanna try to work on that anymore."*

  **The scene is a seam, not a brain.** Every rule that could be wrong is a pure, unit-tested module beside it — `playerController` (1D movement, `CAN_JUMP` false), `jellyLeap` (parametric arc + the fairness proof), `jellyController` (wraps `combat/construct` without forking it), `sideViewScatter` (where a lost hand lands, failing LOUD), plus presenters in `backdrop.ts` / `jellyPresenter.ts`. **Add new behaviour as a new pure module plus a presenter**, never as more inline runtime code — that is the discipline the 3470-line courtyard runtime lost.

  Two combat rules that are not tuning knobs. **The Card-wright has no dodge, roll, shield or jump, ever** — so any enemy attack must be avoidable by *walking*, which `jellyLeap.evaluateEvade()` proves by simulating the exchange at 60Hz. It is a real proof, not a comment: it caught two design bugs before any art existed (cornered against his own castle he had no escape, and a minimum meant for a degenerate hop was overriding every ordinary commit). And **only a telegraphed strong hit scatters the hand**; ordinary damage must not, or losing the cards stops meaning anything.
