        ...
        {filtered.map((card) => (
          <div key={card.cardId} className="relative group">
            ...
            {card.generationModel && (
              <span
                className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded bg-abyss/80 text-[9px] uppercase tracking-widest text-gold/80 font-fantasy pointer-events-none"
                title={`Generated by Leonardo ${card.generationModel}`}
              >
                {card.generationModel.replace(/_/g, ' ')}
              </span>
            )}
            <button
              onClick={(e) => {
