...
export interface Card {
  ...
  /**
   * Which curated roster character this card is a copy of, and which element
   * variant of them. Set by services/forge/curatedMatcher when the card was
   * pulled from the permanent roster.
   *
   * Absence is the LEGACY DISCRIMINATOR: a card without these was generated by
   * the retired runtime pipeline (Claude text + Leonardo portrait). Those cards
   * stay visible and playable forever (Raheem, 2026-08-15) but cannot tier up,
   * because there is no authored rank art to advance them into.
   */
  curatedCharacterId?: string;
  curatedVariantId?: string;
